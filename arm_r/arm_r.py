...
def shoulder_adapter_to_shoulder(sw: Shipwright):
    ...
    return sw.parent(geta_1).rotate(MasterConst.Arm.Right.shoulder_abduction).void()

def shoulder_adapter_to_shoulder_armor(sw: Shipwright):
    return sw.rotate(MasterConst.Arm.Right.shoulder_armer_abduction).void()

def shoulder_to_upper_arm(sw: Shipwright):
    return sw.rotate(0.,MasterConst.Arm.Right.upper_arm_rotation_outer).void(MasterConst.Arm.Shoulder.shoulder_depth)

def upper_arm_to_forearm(sw: Shipwright):
    geta_1 = sw.void(MasterConst.Arm.UpperArm.module_length)
    return sw.parent(geta_1).rotate_x(MasterConst.Arm.Right.forearm_extension)

def forearm_to_hand(sw: Shipwright):
    geta_1 = sw.rotate(0.,MasterConst.Arm.Right.hand_rotation_outer)\
        .void(MasterConst.Arm.ForeArm.module_length-MasterConst.Arm.Hand.to_forearm_overrap)
    geta_2 = sw.parent(geta_1).rotate(MasterConst.Arm.Right.hand_radial_flexion).void()
    return sw.parent(geta_2).rotate_x(MasterConst.Arm.Right.hand_palmar_flexion)

def hand_to_right_weapon(sw: Shipwright):
    geta_1 = sw.void(MasterConst.Arm.Hand.glasp_position_z)
    return sw.parent(geta_1).move_y(MasterConst.Arm.Hand.glasp_position_y)

def modeling_arm_r(sw: Shipwright, path):
    ...
    hand = sw.parent(forearm_to_hand(sw.parent(forearm, 0.)))\
        .load_submodule(os.path.join(path, 'hand_r'), force_load_merged_stl=True, vertex_matching=False)
