...
def thigh_to_shin(sw: Shipwright):
    ...
    return sw.parent(geta_1).rotate_x(MasterConst.Leg.Left.shin_flexion)

def shin_to_foot(sw: Shipwright):
    geta_1 = sw.void(MasterConst.Leg.Shin.length)
    geta_2 = sw.parent(geta_1).rotate(-MasterConst.Leg.Left.foot_radial_flexion).void()
    geta_3 = sw.parent(geta_2).rotate_x(MasterConst.Leg.Left.foot_palmar_flexion)
    return sw.parent(geta_3).rotate(0., -MasterConst.Leg.Left.foot_rotation_outer).void()

def modeling_leg_l(sw: Shipwright, path):
