...
def main():
    ...
    leg_r_path = os.path.join(os.sep.join(path.split(os.sep)[:-2]), 'leg_r')
    fname = path.split(os.sep)[-1] + '.stl'

    sw = Shipwright(Dock())

    sw.load_submodule(os.path.join(leg_r_path, 'thigh'), force_load_merged_stl=True, vertex_matching=False)
